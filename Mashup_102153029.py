...
with st.form("my_form"):
    ...
    st.text_input('Name of Artist', help="Enter the name of the artist", key='artist')
    st.slider('Number of Videos', 5, 40, key='num_videos')
    st.slider('Duration of Each Video (in seconds)', 20, 60, key='video_duration')
    st.text_input('Email Address', help="Enter your email address", key='email')
    st.form_submit_button("Generate", key='generate')

    # Regular expression pattern for email validation
    pattern = r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,7}\b'
    if submitted:
        # Form validation and progress bar
        if not st.session_state.artist.strip():
            st.error('Please enter the name of the artist.')
        elif not re.match(pattern, st.session_state.email):
            st.error('Invalid email address! Please try again.')
        else:
            # Progress bar while generating mashup
            ...
            # Generate the mashup
            script(st.session_state.artist, st.session_state.num_videos, st.session_state.video_duration, st.session_state.email)